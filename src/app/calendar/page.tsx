...
export default function CalendarPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl w-full max-w-sm overflow-hidden" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6">
              ...
              <div className="space-y-2 text-sm mb-5">
                {[
                  ["Coach", modal.cls.coach],
                  ["Horario", `${modal.cls.startTime} – ${modal.cls.endTime}`],
                  ["Cupos", `${modal.cls.reservedCount} / ${modal.cls.maxCapacity}`],
                ].map(([label, value]) => (
                  ...
                  <div
                    className={`h-full rounded-full transition-all ${
                      modal.cls.reservedCount >= modal.cls.maxCapacity
                        ? "bg-red-500"
                        : modal.cls.reservedCount / modal.cls.maxCapacity >= 0.7
                        ? "bg-yellow-500"
                        : "bg-green-500"
                    }`}
              ...
              {isReserved(modal.cls.id, modal.dateStr) ? (
                <button
                  onClick={() => handleCancel(modal.cls.id, modal.dateStr)}
                  disabled={actionLoading}
                  className="w-full py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white font-semibold text-sm transition-colors disabled:opacity-50"
                >
                  {actionLoading ? "Procesando..." : "Cancelar Reserva"}
                </button>
              ) : modal.cls.reservedCount >= modal.cls.maxCapacity ? (
                <button disabled className="w-full py-2.5 rounded-xl bg-zinc-800 text-zinc-500 font-semibold text-sm cursor-not-allowed">
                  Clase Llena
                </button>
              ) : (
                <button
                  onClick={() => handleReserve(modal.cls.id, modal.dateStr)}
                  disabled={actionLoading}
      ...
      {/* ─── Toast ───────────────────────────────────────────────────────── */}
      {toast && (
        <div className={`fixed bottom-6 right-6 px-4 py-3 rounded-xl text-sm font-medium z-50 shadow-2xl transition-all ${

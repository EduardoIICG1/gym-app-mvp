...
function MemberProfileContent({
  ...
  viewUserId: string;
  activeUser: ReturnType<typeof useCurrentUser>;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} className="flex items-start gap-5 mb-8">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          {safeMemberships.length > 0 && (() => {
            const cycles = safeMemberships.flatMap((ms) => {
              try { return [computeMembershipCycle(ms, safeReservations, safeClasses)]; }
              catch { return []; }
            });
                    ...
                    return (
                      <div key={cycle.cycleId}>
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className="text-xs px-2 py-0.5 rounded font-semibold"
                            style={cycle.status === "active" ? { background: "#22c55e20", color: "#22c55e" }
                              : cycle.status === "expired" ? { background: "#ef444420", color: "#ef4444" }
                              : { background: "#71717a20", color: "#71717a" }}>
                            ...
                            {isExpanded && (
                              <div className="mt-3 space-y-0">
                                {cycle.entries.map((entry) => (
                                  <div key={entry.reservationId} className="flex items-center justify-between py-2"
                                    ...
                                    <div className="flex items-center gap-1.5">
                                      ...
                                      <span className="text-xs px-2 py-0.5 rounded font-semibold"
                                        style={entry.displayStatus === "attended" ? { background: "#22c55e20", color: "#22c55e" }
                                          : entry.displayStatus === "absent" ? { background: "#ef444420", color: "#ef4444" }
                                          : entry.displayStatus === "pending_attendance" ? { background: "#f59e0b20", color: "#f59e0b" }
              ...
              <div className="space-y-0">
                {past.map((r) => (
                  <div key={r.id} className="flex items-center justify-between py-2.5" style={{ borderBottom: "1px solid var(--card-border)" }}>
                    ...
                    <span className="text-xs px-2 py-0.5 rounded font-semibold"
                      style={r.status === "attended" ? { background: "#22c55e20", color: "#22c55e" }
                        : r.status === "absent" ? { background: "#ef444420", color: "#ef4444" }
                        : { background: "#71717a20", color: "#71717a" }}>
          ...
          {(safeHealthSessions.length > 0 || safeHealthRestrictions.length > 0) && (() => {
            ...
            return (
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}
                className="rounded-xl border overflow-hidden" style={{ background: "var(--card)", borderColor: "var(--card-border)" }}>
                <div className="px-5 pt-5 pb-4" style={{ borderBottom: "1px solid var(--card-border)" }}>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {!isOwnProfile && (
                      <Link href={`/health/patients/${viewUserId}`} className="text-xs font-semibold hover:underline" style={{ color: "#10b981" }}>
                        Ver ficha clínica →
                      </Link>
                    )}
                  </div>
                    ...
                    <div className="mb-3">
                      ...
                      {totalSessions != null && (
                        <div className="w-full h-2 rounded-full overflow-hidden" style={{ background: "var(--card-border)" }}>
                          <div className="h-full rounded-full transition-all"
                            style={{ width: `${pct}%`, background: pct >= 100 ? "#ef4444" : pct >= 80 ? "#f59e0b" : "#10b981" }} />
                        </div>
                      )}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {safeHealthRestrictions.map((r) => (
                          <span key={r.id} className="text-xs px-2 py-0.5 rounded-lg font-medium"
                            style={r.severity === "critical" ? { background: "#ef444420", color: "#ef4444" }
                              : r.severity === "warning" ? { background: "#f59e0b20", color: "#f59e0b" }
                              : { background: "#4fc3f720", color: "#4fc3f7" }}>
                            {r.label}
                          </span>
                        ))}
                        ...
                        <div key={s.id} className="py-3 flex gap-4"
                          ...
                          <div className="shrink-0 text-right min-w-[72px]">
                            ...
                            <span className="text-[10px] px-1.5 py-0.5 rounded font-semibold"
                              style={s.status === "closed" ? { background: "#71717a20", color: "#71717a" }
...
export default function ProfileClient({
  ...
  const viewUserId = initialViewUserId ?? activeUser.id;
  const isOwnProfile = viewUserId === activeUser.id;

  // Staff own profile — send to /staff-profile instead of crashing
  if (isOwnProfile && ["admin", "coach", "kinesiologist"].includes(activeUser.role)) {
    return (
      <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="rounded-xl p-6 border" style={{ background: "var(--card)", borderColor: "var(--card-border)" }}>
          <p className="text-sm mb-4" style={{ color: "var(--text-secondary)" }}>
            Este perfil corresponde a una cuenta de staff.
          </p>
          <Link
            href="/staff-profile"
